...
int main(int argc, char** argv) {
    monotonicInit();
    if (strstr(monotonic_info_string, "TSC") == NULL) {
        fprintf(stderr, "try use `gcc -DUSE_PROCESSOR_CLOCK main.c` to enable the TSC support if necessary.\n");
    }
    fprintf(stdout, "monotonic_info_string: %s\n", monotonic_info_string);
    monotime mtus = getMonotonicUs();
    fprintf(stdout, "current monotime us: %lu\n", mtus);
    return 0;
}
